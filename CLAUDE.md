...
- `src/mcp.ts` — MCP server with channel capability, cron CRUD tools, notification helper
- `src/config.ts` — Loads config from `~/.clawback/config.json`
- `src/cron/store.ts` — Persistent JSON storage with atomic writes
- `src/cron/scheduler.ts` — Wraps croner library, fires prompt notifications
- `src/cron/types.ts` — CronDefinition interface
- `src/webhook/server.ts` — Bun.serve HTTP server with batching, skill mapping
